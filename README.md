# Deploying Flask to AWS ECS with Terraform
...
1. Create two ECR repositories, `flask-app` and `nginx`.

1. Fork/Clone

1. Build the Flask and Nginx Docker images and push them up to ECR:

    ```sh
    $ cd app
    $ docker build -t <AWS_ACCOUNT_ID>.dkr.ecr.us-west-1.amazonaws.com/flask-app:latest .
    $ docker push <AWS_ACCOUNT_ID>.dkr.ecr.us-west-1.amazonaws.com/flask-app:latest
    $ cd ..
